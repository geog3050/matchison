...
def import_data(filename):
    ...
    with open(filename, "r") as f:
        ...
        for row in csv_reader:
            participants.append([row[0],row[1],float(row[2]),float(row[3])])
    return participants
    f.close()
...
def fight(participant1, participant2, first2attack):
    ...
    ######################
    rounds  = 0
    variable1 = participant1[2] #HP of first participant
    variable2 = participant2[2] #HP of second participant
    
    while variable1 > 0 and variable2 > 0:
        if first2attack == 1:
            variable2 -= attack_multiplier(participant1[1], participant2[1]) * participant1[3]
            first2attack += 1
        elif first2attack == 2:
            variable1 -= attack_multiplier(participant2[1], participant1[1]) * participant2[3]
            first2attack -= 1
        rounds += 1
...
# #Test output
# test_input_participants = import_data('test_input.csv')
# print(tournament(test_input_participants))